            ...
            class h extends s.b {
                ...
                genPoint(t, e, i) {
                    ...
                    if (this.points || (this.points = []), this.options.showDots) {
                        const t = new r.SphereGeometry(.20, 12, 12),
                ...
                onInit() {
                    this.cont = new r.Group, this.cont.position.set(-35, -20, -10), this.scene.add(this.cont);
                    ...
                    h.setAttribute("position", new r.BufferAttribute(this.linePositions, 3).setUsage(r.DynamicDrawUsage)), h.setAttribute("color", new r.BufferAttribute(this.lineColors, 3).setUsage(r.DynamicDrawUsage)), h.computeBoundingSphere(), h.setDrawRange(0, 0);
                    ...
                    this.camera = new r.PerspectiveCamera(20, this.width / this.height, .01, 1e4), this.camera.position.set(50, 23, 150), this.scene.add(this.camera);
                    ...
                    this.linesMesh2 = new r.LineSegments(u, l), this.linesMesh2.position.set(0, -15, 0), this.cont2.add(this.linesMesh2);
                    const p = new r.LineBasicMaterial({
                            color: this.options.color2,
                            linewidth: 2
                        }),
                        d = new r.Geometry;
                    d.vertices.push(new r.Vector3(0, 100, 0)), d.vertices.push(new r.Vector3(0, -100, 0));
                    for (let t = 0; t < 4; t++) {
                    ...
                    this.linesMesh3 = new r.LineSegments(d, p), this.linesMesh3.position.set(0, -15, 0), this.cont2.add(this.linesMesh3);
                    const m = new r.LineBasicMaterial({
                            color: this.options.color
                        }),
                        f = new r.SphereGeometry(16 * this.options.size, 18, 14),
                        w = new r.EdgesGeometry(f);
                    this.sphere = new r.LineSegments(w, m), this.sphere.position.set(0, -15, 0), this.cont2.add(this.sphere), this.cont2.rotation.x = -.25
                }
                onUpdate() {
                    let t;
                    ...
                    for (let t = 0; t < this.points.length; t++) {
                        ...
                        c.scale.z = (.25 * (15 - u)).clamp(1, 100), c.scale.x = c.scale.y = c.scale.z, c.position.y = 2 * Math.sin(c.position.x / 10 + .01 * this.t + c.position.z / 10 * .5);
                        for (let n = t; n < this.points.length; n++) {
                            ...
                            if ((e = Math.sqrt(u * u + p * p + d * d)) < this.options.maxDistance) {
                                ...
                                u = u.clamp(0, 1), n = "additive" === this.blending ? new r.Color(0).lerp(l, u) : h.clone().lerp(a, u), this.linePositions[i++] = c.position.x, this.linePositions[i++] = c.position.y, this.linePositions[i++] = c.position.z, this.linePositions[i++] = t.position.x, this.linePositions[i++] = t.position.y, this.linePositions[i++] = t.position.z, this.lineColors[s++] = n.r, this.lineColors[s++] = n.g, this.lineColors[s++] = n.b, this.lineColors[s++] = n.r, this.lineColors[s++] = n.g, this.lineColors[s++] = n.b, o++
                            }
                        }
                    }
                    return this.linesMesh.geometry.setDrawRange(0, 2 * o), this.linesMesh.geometry.attributes.position.needsUpdate = !0, this.linesMesh.geometry.attributes.color.needsUpdate = !0, this.sphere.material.color.set(a), this.linesMesh2.material.color.set(c), this.linesMesh3.material.color.set(c), .001 * this.t
                }
                onMouseMove(t, e) {
                    const i = this.camera;
